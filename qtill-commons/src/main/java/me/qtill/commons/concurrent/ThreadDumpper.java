...
public class ThreadDumpper {
    ...
    private volatile boolean enable             = true; // 快速关闭该功能（fix，也需要加volatile语义）
    ...
    public void threadDumpIfNeed(String reasonMsg) {
        ...
        synchronized (this) {   // 锁ThreadDumpper实例
    ...
    private String dumpThreadInfo(Thread thread, StackTraceElement[] stackTrace, StringBuilder sb) {
        ...
        sb.append('\n');
        int i = 0;
        int finalStackLevel = Math.min(maxStackLevel, stackTrace.length);
        for (; i < finalStackLevel; i++) {
